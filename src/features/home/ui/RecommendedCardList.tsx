...
export default function RecommendedCardList() {
  ...
  return (
    <section className='mt-[76px] mb-[116px] w-full'>
      <div className='mx-auto w-full max-w-[1440px]'>
        ...
        <div className='grid grid-cols-1 gap-x-[40px] gap-y-[30px] sm:grid-cols-2 lg:grid-cols-3'>
          {projects.map((projects, idx) => (
            <div
              key={idx}
              className='flex h-[250px] flex-col justify-between rounded-lg border border-zinc-200 px-6 pt-[27px] pb-[30px] shadow-sm transition hover:shadow-lg'
            >
              <div>
                <p className='text-sm text-zinc-400'>마감일 | {projects.end_date}</p>
                <h3 className='mt-[20px] line-clamp-2 text-[24px] font-medium'>
                  {projects.title}
                </h3>
              </div>
