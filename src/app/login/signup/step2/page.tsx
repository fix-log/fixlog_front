    ...
    return (
        <div className="flex flex-col items-center !max-w-[500px] w-full">
            ...
            <form className="w-full" onSubmit={form.handleSubmit(handleClick)}>
                <InputString label="이름" type="text" name="name" placeholder="이름을 입력해주세요" form={form} />
                <InputString label="생년월일" type="text" name="birthDate" placeholder="YYYYMMDD" form={form} />
                <FormRadioSelect label="성별" name="gender" button={['남성', '여성']} form={form} />
                <InputString label="전화번호" type="text" name="phoneNumber" placeholder="숫자만 입력해주세요" form={form} />
