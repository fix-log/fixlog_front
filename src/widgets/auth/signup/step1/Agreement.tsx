...
const DATA = [
  { id: 'isOver14Agreed', text: '만 14세 이상입니다', isRequired: 'required' },
  { id: 'isTermsAgreed', text: '서비스 이용약관', isRequired: 'required' },
  { id: 'isPrivacyAgreed', text: '개인정보 수집 및 이용', isRequired: 'required' },
  { id: 'isMarketingAgreed', text: '이벤트, 프로모션 알림 메일 수신', isRequired: 'optional' },
];
      ...
      <div>
        {DATA.map((item) => (
          <TermsAgreement
            ...
            id={item.id}
            text={item.text}
            isRequired={item.isRequired as 'required' | 'optional' | undefined}
          />
        ))}
      </div>
    </div>
